...
- [ ] **Give the sharpened eval case 2 a rate** — it was sharpened on 2026-09-09 and discriminates on its first run (6/6 with the plugin, 4/6 on an isolated baseline), but one run is not a rate and the old 3/3 does not carry to a changed case. Three runs of each arm makes its `docs/AGENT-EVALS.md` entry `probed` again ([ADR 0043](docs/decisions/0043-probed-not-validated.md)).
...
- [x] **This repo's own project-scope install was two versions behind** — `installed_plugins.json` pinned `groundrules@claude-code-groundrules` at 1.10.0 for this directory, so the dogfood loaded a plugin without `close` or any 1.11.0 change. Found by eval case 2. Both scopes are now at **1.12.0** (`claude plugin update --scope project`; `install` refuses, it only reports the plugin as already present). — 2026-09-21

- [x] **Three stale items removed from `In progress`** — two empty markers and a closed one left as unchecked bullets by successive edits, which had also swallowed the case-2 item they were meant to supersede. The record claimed work that was done and hid work that was not: exactly what `/groundrules:close` exists to catch, produced by hand-editing the file it checks. — 2026-09-21
