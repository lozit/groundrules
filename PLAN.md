...
- [ ] **M1 — Loop-readiness OPENED** (2026-06-14). Building brick by brick (each its own PRD). Order: **(1) minimal runnable loop ✅** → **(2) wire scaffolding into bootstrap/adopt** ← *next* → (3) `/groundrules:realize` → (4) TDD-gate → (5) backward-crossing → (6) template refinement. Full milestone in [`docs/ROADMAP.md`](docs/ROADMAP.md) (M1).
  ...
  - [ ] **Brick 2 — Wire opt-in loop scaffolding into `bootstrap`/`adopt`** (PRD `docs/prd/loop-scaffolding-bootstrap-adopt.md`, **draft**). Port the validated prototype to `{{KEY}}` templates (carry the 3 contract fixes) + `## Invariants` in `CLAUDE.md`; opt-in once-per-project, off by default, defer-to-superpowers; `.groundrules.json` flag; fresh-subagent E2E. **All 5 open questions resolved** 2026-06-14: the 2 structural ones (artifact location + backlog) settled in [ADR 0030](docs/decisions/0030-loop-namespace-and-backlog.md) — a top-level `loop/` namespace, loop reads `loop/backlog.md`, `PLAN.md` only points to it; the 3 others = dedicated opt-in question · Invariants conditional on opt-in · superpowers → don't offer. *(reflection done → PRD ready to build pending go)*
