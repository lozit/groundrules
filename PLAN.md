...
- [ ] **M1 — Loop-readiness OPENED** (2026-06-14). Building brick by brick (each its own PRD). Order: **(1) minimal runnable loop ✅** → **(2) wire scaffolding into bootstrap/adopt ✅** → **(3) `/groundrules:realize` ✅** → **(4) TDD-gate ✅** → **(5) backward-crossing ✅** → **(6) template refinement ✅**. **All 6 bricks done → M1 ready for release.** Full milestone in [`docs/ROADMAP.md`](docs/ROADMAP.md) (M1).
  - [x] **Brick 1 — Minimal runnable loop** ✅ (PRD `docs/prd/loop-minimal-runnable.md`, validated 2026-06-14). Prototype in `docs/prototypes/loop/` (maker/verifier + LOOP.md + capped runner + blocked.md + slugify fixture). Subagent-simulated all three paths: converge · adversarial REJECT of a green-but-gamed diff · BLOCKED-not-guess. Verdict = productize, with contract fixes (separate-context verifier; reject-too-weak-test; stage-the-diff-not-`git add -A`) → `docs/LEARNINGS.md`. Under `[Unreleased]`.
  - [x] **Brick 6 — Template refinement (route work by regime)** ✅ (PRD `docs/prd/template-refinement-regimes.md`). Refined the generated `CLAUDE.md` "Claude Code workflow": flat "plan mode before any non-trivial task" → the ADR-0027 routing list (decision→ADR · feature→PRD · interactive non-trivial→plan mode · atomic/testable→realize→loop, the loop route an inline-conditional one-liner). One-section wording refinement; no leftover flat phrasing; lean preserved. Validated statically (no `HAS_LOOP` branching on this line — identical loop/non-loop output). README CHANGELOG note. Under `[Unreleased]`.
  - [ ] **M1 release** — sweep `generated-by` signatures to the new version, bump `plugin.json` + `marketplace.json`, finalize `## [Unreleased]` → `## [X.Y.Z]`, `.groundrules.json` migration entry, README review (skill-list 13/13 + justification drift), then commit + push + `git tag` + `gh release`. Decide the version bump (loop-readiness is a substantial feature set → likely minor **1.6.0**, confirm).
  - [x] **Brick 5 — Backward-crossing convention** ✅ (PRD `docs/prd/backward-crossing-convention.md`). **Convention, not a skill** (per ADR 0027). "Triaging `loop/blocked.md`" section in `loop/README.md.tpl` (3 routes + tie-breaker + the anti-guess guard + greppable `Resolution:` close-out + repo-gets-smarter); `loop/maker.md` writes `Resolution: (open — awaiting triage)`; one-line CLAUDE.md breadcrumb (loop-conditional); README/CHANGELOG. **Fresh-subagent E2E** triaging 3 planted blockers (too-big→re-decompose · decision→ADR-not-guessed · visual→interactive): all routed correctly, decision not invented; caught 4 spec gaps, all fixed (the key one: the anti-guess guard lived only in maker.md, not the triage convention — an autonomous triager could read "make the call" as license; + routed-but-pending ≠ resolved · multi-category tie-breaker · greppable Resolution). LEARNINGS entry. Under `[Unreleased]`.
  - [x] **Brick 4 — TDD-before-loop gate** ✅ (PRD `docs/prd/tdd-gate.md`). Hardened `realize`'s `[loop]` bar to **a pre-written behavioural acceptance test, writer ≠ maker, currently RED** (build/lint insufficient; green/maker-authored doesn't qualify); interactive author-the-test step (draft/strengthen → confirm red → `[loop]`, else `[supervised]`). Sharpened maker (authoritative immutable acceptance test vs assistive unit tests) + verifier (replays the acceptance test); `PRD.md.tpl` note + loop-regime-only TDD line; README/CHANGELOG. **Fresh-subagent E2E** on 4 candidates (no test → author red → loop · green-trivial → refused · real red → loop · build/lint → supervised): all correct; caught 5 spec gaps, fixed 4 (strengthen-weak-test procedure · red-for-the-right-reason behavioural vs import · gate≠spec-validation caveat · green=done-vs-weak), deferred 1 (supervised routing taxonomy — inline reason suffices). LEARNINGS entry. Under `[Unreleased]`.
  - [x] **Brick 3 — `/groundrules:realize`** ✅ (PRD `docs/prd/realize-skill.md`). New skill `skills/realize/SKILL.md`: ingest a plan (paste · PRD build plan · PLAN.md) → partition `[loop]`/`[supervised]` against a 5-point bar, **refuse `[loop]` without a re-runnable stop condition**, explicit confirm (no auto-promote), write `[loop]` → `loop/backlog.md` (maker format, append via Edit, idempotent) + tag `[supervised]` in PLAN.md with reason; preconditions (scaffolded? superpowers→defer). README workflow 13/13, CHANGELOG. **Fresh-subagent E2E** on a 7-task adversarial mixed plan: partition correct, **nothing decision-laden/unverifiable slipped into `[loop]`**, refusals + decomposition correct; caught 7 spec gaps (all fixed) incl. a systemic one — `allowed-tools` must include `Edit` for append-skills (also fixed bootstrap's `## Invariants` insertion to compose-before-write). LEARNINGS entry added. ⚠️ new skill dir → needs a Claude Code restart to register. Under `[Unreleased]`.
  - [x] **Brick 2 — Wire opt-in loop scaffolding into `bootstrap`/`adopt`** ✅ (PRD `docs/prd/loop-scaffolding-bootstrap-adopt.md`, [ADR 0030](docs/decisions/0030-loop-namespace-and-backlog.md)). 8 `loop/` templates (maker/verifier/LOOP/runner/backlog/README/gitignore + Invariants snippet) carrying the 3 brick-1 contract fixes; bootstrap Call 2c + adopt Call 3c (dedicated, off by default, defer-to-superpowers, resume-skip); `## Invariants` conditional; `.groundrules.json` `loop.scaffolded`; README/CHANGELOG synced. **Fresh-subagent E2E** (generate + resume): generation correct (17 files, Invariants placed, runner 755, no stray `{{KEY}}`), resume-safe; caught 4 loop instruction gaps + 2 pre-existing bugs (stale `v1.3.3` commit msg, FR comment in `gitignore.minimal`), all fixed. Under `[Unreleased]`.
