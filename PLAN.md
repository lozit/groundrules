...
- [ ] **M1 — Loop-readiness OPENED** (2026-06-14). Building brick by brick (each its own PRD). Order: **(1) minimal runnable loop ✅** → **(2) wire scaffolding into bootstrap/adopt ✅** → **(3) `/groundrules:realize` ✅** → **(4) TDD-gate** ← *in progress (PRD draft)* → (5) backward-crossing → (6) template refinement. Full milestone in [`docs/ROADMAP.md`](docs/ROADMAP.md) (M1).
  ...
  - [ ] **Brick 4 — TDD-before-loop gate** (PRD `docs/prd/tdd-gate.md`, **draft**). Harden `realize`'s `[loop]` bar from "a stop condition exists" → **a pre-written acceptance test, writer ≠ maker, currently RED** (red-first proves it constrains; independence stops self-grading). Add an interactive author-the-test step (mini-spec → failing test → confirm red) else stays `[supervised]`. Sharpen maker/verifier wording (authoritative acceptance test vs assistive unit tests); `PRD.md.tpl` Success-criteria note; loop-regime-only TDD (NOT global — handoff-not-gospel). **All 4 open questions resolved** 2026-06-14: tighten to a behavioural acceptance test (build/lint insufficient) · guide+draft but gate on a red test existing · accept user-confirmed red · test lives in the project's test dir. *(reflection done → PRD ready to build pending go)*
