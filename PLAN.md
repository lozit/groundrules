...
- [ ] **M1 — Loop-readiness OPENED** (2026-06-14). Building brick by brick (each its own PRD). Order: **(1) minimal runnable loop ✅** → **(2) wire scaffolding into bootstrap/adopt ✅** → **(3) `/groundrules:realize` ✅** → **(4) TDD-gate ✅** → **(5) backward-crossing** ← *in progress (PRD draft, convention chosen)* → (6) template refinement. Full milestone in [`docs/ROADMAP.md`](docs/ROADMAP.md) (M1).
  ...
  - [ ] **Brick 5 — Backward-crossing convention** (PRD `docs/prd/backward-crossing-convention.md`, **draft**). **Convention, not a skill** (fork decided 2026-06-14, per ADR 0027 — resolution work already in add-adr/realize/interactive; skill deferred). Codify "Triaging `loop/blocked.md`": 3 routes (re-decompose→realize / decide→add-adr→re-add / fix interactive) + when each + a `Resolution:` field (audit trail, not graveyard) + repo-gets-smarter principle. Templates/docs only: `loop/README.md.tpl` section, `loop/maker.md` blocked.md format, a one-line CLAUDE.md breadcrumb. **All 3 open questions resolved** 2026-06-14 on their leans (CLAUDE.md breadcrumb yes · Resolution stub written by maker · no auto-unpark). *(reflection done → PRD ready to build)*
