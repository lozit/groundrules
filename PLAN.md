...
- [ ] **M1 — Loop-readiness OPENED** (2026-06-14). Building brick by brick (each its own PRD). Order: **(1) minimal runnable loop ✅** → **(2) wire scaffolding into bootstrap/adopt ✅** → **(3) `/groundrules:realize`** ← *in progress (PRD draft)* → (4) TDD-gate → (5) backward-crossing → (6) template refinement. Full milestone in [`docs/ROADMAP.md`](docs/ROADMAP.md) (M1).
  ...
  - [ ] **Brick 3 — `/groundrules:realize`** (PRD `docs/prd/realize-skill.md`, **draft**). Forward bridge: ingest an approved plan → propose a partition `[loop]`/`[supervised]` with rationale, **refuse `[loop]` without a re-runnable stop condition**, write `[loop]` → `loop/backlog.md` (maker-consumable, resume-safe), mark `[supervised]` in `PLAN.md`. Conservative + explicit confirm (no auto-promote); preconditions (loop scaffolded? superpowers→defer). **All 5 open questions resolved** 2026-06-14: load-bearing brick-3/brick-4 boundary = brick 3 only checks a stop condition *exists*, the TDD acceptance-test gate is brick 4; + input source (all 3, ask) · no-scaffolding → inline gen or pointer · `[supervised]` tagged in place · no `.groundrules.json` state V1. *(reflection done → PRD ready to build pending go)*
