...
- [ ] **M1 — Loop-readiness OPENED** (2026-06-14). Building brick by brick (each its own PRD). Order: **(1) minimal runnable loop ✅** → **(2) wire scaffolding into bootstrap/adopt ✅** → **(3) `/groundrules:realize` ✅** → **(4) TDD-gate ✅** → **(5) backward-crossing ✅** → **(6) template refinement** ← *in progress (PRD draft, last brick before release)*. Full milestone in [`docs/ROADMAP.md`](docs/ROADMAP.md) (M1).
  ...
  - [ ] **Brick 6 — Template refinement (route work by regime)** (PRD `docs/prd/template-refinement-regimes.md`, **draft**). Sharpen the generated `CLAUDE.md` "Claude Code workflow" from a flat "plan mode before any non-trivial task" into the ADR-0027 triad: decision→ADR · interactive non-trivial→plan mode · feature→PRD · (loop-scaffolded) atomic/testable→realize→loop. Wording refinement of one section only — no new skill/doc/runtime. **Both open questions resolved** 2026-06-14 on their leans (loop route = inline-conditional one-liner · keep workflow section collapsible). *(reflection done → PRD ready to build)*
